...
static void gtk_dial_dispose                  (GObject          *object);
static void gtk_dial_realize                  (GtkWidget        *widget);
static void gtk_dial_size_request             (GtkWidget        *widget,
                                               GtkRequisition   *requisition);
static void gtk_dial_size_allocate            (GtkWidget        *widget,
                                               GtkAllocation    *allocation);
static gboolean gtk_dial_show               (GtkWidget        *widget,
...
gtk_dial_class_init (GtkDialClass *class)
{
  ...
  object_class->dispose = gtk_dial_dispose;

  widget_class->realize = gtk_dial_realize;
#ifdef dmg
  widget_class->show_event = gtk_dial_expose;
...
{
  dial->button = 0;
  dial->policy = GTK_DIAL_UPDATE_CONTINUOUS;
  dial->timer = 0;
  dial->radius = 0;
...
static void
gtk_dial_dispose (GObject *object)
{
  GtkDial *dial;
  ...
  if (dial->button == event->button)
    {
      ...
      if (dial->policy == GTK_DIAL_UPDATE_DELAYED)
	g_source_remove (dial->timer);
      
      if ((dial->policy != GTK_DIAL_UPDATE_CONTINUOUS) &&
